...
const AmountButton = ({
  ...
  return (
    <button
      onClick={click}
      className={`${current ? 'bg-canopySelected border-canopySelectedBorder border-[1px solid] text-canopyTextColor' : 'bg-canopyBase text-canopyTextColor'} border rounded-sm m-2 px-2 py-3 min-w-0`}
    >
      {typeof amount === 'string' ? `${amount}` : `$${amount.toLocaleString()} USD`}
    </button>
  );
};

export const CanopyDonationPresets = ({
  presetAmounts,
  setAmount,
  recurring,
  currentAmount,
  ...
  const defaultAmounts = [40, 75, 100, 200];
  ...
  return (
    <>
      <div className="grid grid-cols-2 font-lato text-sm tracking-wider font-bold max-tiny:grid-cols-2">
      ...
      <div className="flex justify-center font-lato text-sm font-bold">
        <button
          onClick={() => {
            setOtherAmount(true);
            setAmount(1);
          }}
          className={`${otherAmount ? 'bg-canopySelected border-canopySelectedBorder border-[1px solid] text-canopyTextColor' : 'bg-canopyBase text-canopyTextColor'} border rounded-sm m-2 px-2 py-3 min-w-0 w-full`}
        >
          Other Amount
        </button>
      </div>
